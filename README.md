# AI Blog CMS v2.5 - Auto-Reference Published Articles
...
- **自動参照機能** - 公開記事が自動的に参照データとして利用可能（v2.5新機能）
- **内部リンク管理** - マインドマップで記事間のつながりを可視化（v2.4）

## 🚀 公開URL

### 本番環境
- **管理画面**: https://421170e4.ai-blog-cms-471.pages.dev/admin
- **公開ブログ**: https://421170e4.ai-blog-cms-471.pages.dev/blog
...
### v2.5 (2025-11-25)
- ✅ **参照データ自動追加機能** - 公開記事が自動的に参照データとして利用可能に
- ✅ 自動追加記事には特別なバッジ表示
- ✅ 自動追加記事は読み取り専用（記事管理から編集）
- ✅ 公開ページへのリンクを参照データに追加
- ✅ カテゴリーフィルターで「過去記事」を選択可能

### v2.4 (2025-11-24)
- ✅ **内部リンク管理マインドマップ** - Vis.jsで記事間のつながりを可視化
- ✅ 記事・H2・H3見出しをノードとして表示
- ✅ ドラッグ＆ドロップで接続を作成
- ✅ トピッククラスター構造の視覚化
...
### v2.2 (2025-11-22)
- ✅ **プロンプト管理強化** - 複数プロンプトの追加・削除が可能に
- ✅ アウトライン生成・記事生成で複数プロンプトを選択可能
...
**公開記事の自動追加**:
- 公開ステータスの記事が自動的に参照データとして表示されます
- カテゴリー「過去記事」でフィルタリング可能
- 自動追加記事には<span style="background: #3B82F6; color: white; padding: 2px 6px; border-radius: 4px;">🔮 自動追加</span>バッジが表示されます

**利用方法**:
1. サイドバーから「参照データ管理」をクリック
2. 公開済み記事が自動的に一覧に表示されます
3. 「過去記事」フィルターで公開記事のみ表示
4. 記事をクリックして内容を確認・コピー可能
5. AI生成時の参考資料として活用

**注意事項**:
- 自動追加記事は編集・削除できません
- 編集が必要な場合は「記事管理」から行ってください
- 記事のステータスを「公開」から変更すると、参照データからも削除されます

### 内部リンク管理（v2.4）

**マインドマップでの可視化**:
- 記事・H2・H3見出しがノードとして表示
- ドラッグ＆ドロップで接続を作成
- トピッククラスター構造を視覚的に管理

**利用方法**:
1. サイドバーから「内部リンク管理」をクリック
2. マインドマップで記事間のつながりを確認
3. 「接続を追加」ボタンで新しいリンクを作成
4. 公開ページに自動的に内部リンクカードが挿入されます
